...
def zaj01_skrypt4():
    ...
    print(
        "Cala zawartosc zmiennej po zmodyfikowaniu tak by w każdym wyrazie ostatni znak był w formacie dużej litery:"
    )
    print(teksty)
    print("Typ zmiennej po zmianach: " + str(type(teksty)))
    print()
    ...
    print(
        "Zmienna, ktora przetrzymuje ilosc wystapien dla kazdego ze slow wystepujacych w odpowiednim tescie: "
    )
    ...
    print(
        "Zmienna, ktora przetrzymuje ilosc wystapien dla kazdego ze slow wystepujacych w tekstach: "
    )
    print(il_poszczegolnych_slow_calosc)

    with open(
        "./cw1/zadanie_sprawdzajace/zadanie_spr.json", "w", encoding="utf-8"
    ) as plik:
        json.dump(unikalne_slowa, plik, ensure_ascii=False, indent=4)

    with open(
        "./cw1/zadanie_sprawdzajace/zadanie_spr.json", "a", encoding="utf-8"
    ) as plik:
