    ...
    def assign_ambulances(self):
        for incident in self.incidents:
            ...
            if available_ambulances:
                best_ambulance = min(available_ambulances, key=lambda a: a.distance_to(incident.location))
                if best_ambulance.assign_to_incident(incident):
                    incident.status = "assigned"
                    print(f"Assigned Ambulance {best_ambulance.id} to Incident {incident.id}")
